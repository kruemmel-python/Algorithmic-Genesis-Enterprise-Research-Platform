...
namespace ag_sequence_extrapolator_cf5cb9a3f619 {
...
inline double kernel(double x){ x = sanitize(x); return sanitize(((ag_logabs(x) - ag_logabs(std::tanh(x))) + std::tanh(std::tanh(x)))); }
...
extern "C" inline double evaluate_safely(const double* inputs, int* gas_left, int* error_code){
    ...
    --(*gas_left); *error_code = 0; return ag_sequence_extrapolator_cf5cb9a3f619::kernel(inputs[0]);
}
