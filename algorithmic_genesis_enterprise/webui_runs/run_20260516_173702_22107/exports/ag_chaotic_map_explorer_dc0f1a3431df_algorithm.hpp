...
namespace ag_chaotic_map_explorer_dc0f1a3431df {
...
inline double kernel(double x){ x = sanitize(x); return sanitize(std::tanh(std::tanh(((x * std::cos(x)) / (x * std::cos(x)))))); }
...
extern "C" inline double evaluate_safely(const double* inputs, int* gas_left, int* error_code){
    ...
    --(*gas_left); *error_code = 0; return ag_chaotic_map_explorer_dc0f1a3431df::kernel(inputs[0]);
}
